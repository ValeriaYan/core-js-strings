...
function isPalindrome(str) {
  ...
  if (str[str.length - 1] === '!' || str[str.length - 1] === '?') {
    myStr = myStr.slice(0, str.length - 1);
  }
  myStr = myStr.replaceAll(' ', '');
  myStr = myStr.replaceAll(',', '');
  for (let i = 0; i < myStr.length / 2; i += 1) {
    if (
      myStr[i].toLowerCase() !== myStr[myStr.length - 1 - i].toLocaleLowerCase()
    ) {
      return false;
    }
  }
...
function invertCase(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (
      str.charCodeAt(i) >= startCharCode &&
      str.charCodeAt(i) <= endCharCode
    ) {
...
function extractNameFromTemplate(value) {
  // throw new Error('Not implemented');
  return value.substring(0, value.length - 1).substring(7);
}
